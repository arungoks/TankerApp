...
@Singleton
class RoomTankerRepository @Inject constructor(
    private val tankerDao: TankerDao
) : TankerRepository {
    /**
     * Returns a flow of tanker logs for the given month.
     */
    override fun getTankersForMonth(yearMonth: YearMonth): Flow<List<TankerLog>> {
     ...
     */
    override fun getTankerCount(date: LocalDate): Flow<Int> {
     ...
     */
    override fun getCurrentCycleTankerCount(): Flow<Int> {
        return tankerDao.getAllTankerLogs().map { logs ->
            logs.sumOf { it.count }
        }
    }
